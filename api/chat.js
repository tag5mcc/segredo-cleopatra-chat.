...
const FREE_TRIAL_MS = 3 * 60 * 1000; // 3 minutos grátis
...
TOM DE VOZ: Confiante, calorosa, um pouco provocadora no bom sentido. Trate a pessoa como "querida" ocasionalmente, sem exagerar. Responda sempre em português do Brasil.
...
async function checkAccess(anonymousId, paidEmail) {
  ...
  if (paidEmail) {
    ...
    try {
      const paid = await redisCmd(['GET', `paid:${normalized}`]);
  ...
  // 2. Sem e-mail válido: aplica a regra do teste grátis de 3 minutos, controlada no servidor.
  if (!anonymousId) {
  ...
  let startStr;
  try {
    startStr = await redisCmd(['GET', key]);
  } catch (e) {
  ...
  if (!startStr) {
    start = Date.now();
    try {
      await redisCmd(['SET', key, String(start), 'EX', String(TRIAL_KEY_TTL_SECONDS)]);
    } catch (e) {
...
module.exports = async function handler(req, res) {
  ...
  const { messages, memory, mode, today, anonymousId, paidEmail } = req.body || {};
